...
const DEFAULT_CONFIGS = {
  'javascript': { language: 'javascript-typescript', scanner: 'codeql' },
  'typescript': { language: 'javascript-typescript', scanner: 'codeql' },
  'python': { language: 'python', scanner: 'codeql' },
  'go': { language: 'go', scanner: 'codeql' },
  'java': { language: 'java-kotlin', scanner: 'codeql', build_mode: 'manual' },
  'cpp': { language: 'cpp', scanner: 'codeql' },
  'csharp': { language: 'csharp', scanner: 'codeql' },
  'ruby': { language: 'ruby', scanner: 'codeql' }
};
...
export function detectLanguages(githubLanguages) {
  ...
  for (const [githubLang, bytes] of Object.entries(githubLanguages)) {
    ...
    if (scannerLang) {
      console.error(`✓ Found ${githubLang} (${bytes} bytes) - adding '${scannerLang}' to scanner list`);
  ...
  console.error(`Language detection summary: Found ${languages.length} languages for CodeQL scanning`);
...
export function createMatrix(detectedLanguages, languagesConfig = []) {
  ...
  console.error('Generated matrix for CodeQL jobs:', JSON.stringify(matrix, null, 2));
  console.error(`Total matrix entries: ${matrixIncludes.length}`);
  console.error('============================');
